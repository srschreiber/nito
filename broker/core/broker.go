...
// Broker is the central server object shared by the WebSocket, HTTP handler,
// and voice subsystems.
type Broker struct {
	Address     string
	DB          *pgxpool.Pool
...
func (b *Broker) writeLoop(ctx context.Context, client *Client) {
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-pingTicker.C:
			if err := client.connection.WriteControl(gorillaws.PingMessage, nil, time.Now().Add(5*time.Second)); err != nil {
		...
		case msg, ok := <-client.send:
			...
			if err := client.connection.WriteMessage(gorillaws.TextMessage, msg); err != nil {
				_ = client.connection.Close()
				return
			}
		}
...
// sendToClient delivers a WS message to a connected user. No-op if the user is not connected.
func (b *Broker) sendToClient(userID, rpcName string, payload json.RawMessage) {
	...
	if err != nil {
		log.Printf("sendToClient marshal: %v", err)
		return
	}
	client.trySend(data)
}
